    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(sender.hasPermission(EssentialsBungeeConstants.ADMIN_CHAT_COMMAND_PERMISSION)){
            if(args.length >=1){
                ...
                ChatUtils.broadcast(EssentialsBungeeConstants.ADMIN_CHAT_LABEL + " " +
                        ChatColor.WHITE+ sender.getName() +": "+ChatColor.DARK_RED + message,
                        EssentialsBungeeConstants.ADMIN_CHAT_COMMAND_PERMISSION);
            }else{
