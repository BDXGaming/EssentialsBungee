    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(sender.hasPermission(EssentialsBungeeConstants.USERINFO_COMMAND_PERMISSION)){
            if(args.length >=1){
                ...
                if(player != null){
                    String message = ChatColor.translateAlternateColorCodes('&', "&e&lPlayer &r&a" + args[0] + "&r&e&l information \n \n");
                    ...
                    message += ChatColor.YELLOW + "Current Server: " +ChatColor.WHITE+ player.getServer().getInfo().getName() + "\n";

                    if(sender.hasPermission(EssentialsBungeeConstants.USERINFO_IP_COMMAND_PERMISSION)){
                        message += ChatColor.YELLOW + "IP Address: " +ChatColor.WHITE+ player.getSocketAddress().toString() + "\n";
                    }

                    sender.sendMessage(new TextComponent(message+ "\n "));

                }else{
                    sender.sendMessage(new TextComponent(ChatColor.DARK_RED + "Player "+ ChatColor.RED + args[0] + ChatColor.DARK_RED + " is not currently online!"));
                }
